  ...
  <div class="modal fade modal-getstart" id="modalFormSuggestion" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        {!! Form::open(['route'=>'admin-post-suggestion','files'=>true, 'class' => 'form-horizontal jquery-form-suggestion']) !!}
...
@section('scripts')
    ...
    <script>
        ...
        var table = $('#teacher-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{!! route('admin-suggestion-datatables') !!}",
                order: [[ 0, 'desc' ]],
                columns: [
                    ...
                    {data: 'status', name: 'status',"render": function (data, type, JsonResultRow, meta) {
                        ...
                        if(JsonResultRow.status == "Not Yet Checked"){
                            ...
                            check = '<b><p style="text-align:right;font-style:italic;color:green;">Checked</p></b>';
                        }
                            return check;
                    }},
        ...
        function show_suggestion(id){
            $.ajax({
                type: "POST",
                url: "{!! route('admin-show-suggestion') !!}",
                data: {
                    'id': id
                },
        ...
        function show_form_update(id){          
            $.ajax({
                type: "POST",
                url: "{!! route('admin-post-suggestion') !!}",
                data: {
                    'id': id,
                    'action': 'get-data'
                },
